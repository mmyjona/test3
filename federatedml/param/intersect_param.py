...
class EncodeParam(BaseParam):
    ...
    def check(self):
        ...
        self.check_and_change_lower(self.encode_method,
...
class IntersectParam(BaseParam):
    ...
    synchronize_intersect_ids: bool. In rsa, 'synchronize_intersect_ids' is True means guest or host will send intersect results to the others, and False will not.
    ...
    def __init__(self, intersect_method=consts.RAW, random_bit=128, synchronize_intersect_ids=True, join_role="guest",
        ...
        self.synchronize_intersect_ids = synchronize_intersect_ids
    ...
    def check(self):
        ...
        if type(self.synchronize_intersect_ids).__name__ != "bool":
            raise ValueError(
                "intersect param's is_send_intersect_ids {} not supported, should be bool type".format(
                    self.synchronize_intersect_ids))
